...
系统 SHALL 按计划中当前启用目的地的比例分配建议定投总额，不限制目的地数量或名称，并保证所有最终金额非负且合计严格等于建议总额。

#### Scenario: Allocate to a subset of default destinations
- **WHEN** 计划只启用支付宝基金和现金且两者比例合计为 100%
- **THEN** 系统只为这两个目的地生成建议金额，不为已停用默认目的地生成新建议

#### Scenario: Allocate to a custom destination
- **WHEN** 计划包含用户新增且已启用的目的地
- **THEN** 系统按照该目的地比例参与计算，行为与默认目的地一致

#### Scenario: Allocate to one destination
- **WHEN** 计划只有一个比例为 100% 的启用目的地
- **THEN** 系统把全部建议总额分配给该目的地

#### Scenario: Allocate a rounding remainder deterministically
- **WHEN** 任意数量目的地向下取整后仍有完整取整单位未分配
- **THEN** 系统按原始分项小数余数从大到小分配剩余单位，并用目的地排序及稳定 ID 处理相同余数
