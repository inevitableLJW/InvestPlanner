...
一个以“先创建计划，再按计划记录月度收支”为核心的 Web 应用。第一版支持账户登录、多定投计划、可配置定投目的地、按付款平台汇总支出、服务端权威计算、实际投入追踪、历史和计划级统计。
...
目的地数量不固定。启用目的地至少一个且比例合计必须为 100%；使用最大余数法分配取整尾差。历史月份保存目的地名称、顺序和比例快照，之后修改计划不会改写历史结构。
